...
export async function startServer(): Promise<void> {
  try {
    ...
    logger.info(`Loaded ${preShippedResult.prompts.length} pre-shipped prompt(s)`);
    if (preShippedResult.errors.length > 0) {
      logger.warn(`Failed to load ${preShippedResult.errors.length} pre-shipped prompt(s)`);
    }
    ...
    if (config.customPromptsDir) {
      ...
      const customResult = await PromptLoader.loadFromDirectory(config.customPromptsDir);
      ...
      if (customResult.errors.length > 0) {
        logger.warn(`Failed to load ${customResult.errors.length} custom prompt(s)`);
      }
    }
    ...
    if (totalPrompts === 0) {
      logger.warn('No prompts loaded! Server will start but have no prompts to serve.');
    }
    ...
    if (transports.length === 0) {
      logger.error('No transports enabled! Enable at least one transport (stdio or HTTP).');
      process.exit(1);
    }

    logger.info('MCP Prompts Server started successfully!');
    logger.info(`Serving ${totalPrompts} prompt(s) via ${transports.length} transport(s)`);
