...
describe("PromptLoader", () => {
  describe("directoryExists()", () => {
    ...
    it("should return false for non-existent directory", async () => {
      const exists = await PromptLoader.directoryExists(
  ...
  describe("loadPromptFile()", () => {
    ...
    it("should throw validation error for invalid prompt", async () => {
      ...
      await expect(
        PromptLoader.loadPromptFile(filePath, "custom")
  ...
  describe("loadFromDirectory()", () => {
    it("should load all valid prompts from directory", async () => {
      ...
      const validPrompt = result.prompts.find(
        (p) => p.name === "test-prompt-1"
      );
    ...
    it("should collect errors for invalid files", async () => {
      ...
      const invalidError = result.errors.find((e) =>
        e.filePath.includes("invalid-prompt.md")
      );
