...
describe('Prompt Parameter Substitution', () => {
  ...
  it('should substitute simple template variables', async () => {
    ...
    expect(jsonData.result.messages[0].content.text).toBe('Hello Alice! Welcome to Wonderland.');
  });
...
{{/if}}`,
    ...
    const dataLineAll = textResponseAll.split('\n').find(line => line.startsWith('data: '));
    ...
    const dataLineReq = textResponseReq.split('\n').find(line => line.startsWith('data: '));
