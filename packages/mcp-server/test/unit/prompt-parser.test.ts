...
import {
  parsePromptFile,
  parsePromptFiles
} from "../../src/prompts/prompt-parser.js";
...
describe("parsePromptFile", () => {
  it("should parse valid prompt with all fields", () => {
    const content = readFileSync(
      join(FIXTURES_DIR, "valid-prompt.md"),
      "utf-8"
    );
    const result = parsePromptFile(
      "/test/valid-prompt.md",
      content,
      "pre-shipped"
    );
  ...
  it("should parse minimal prompt with only required fields", () => {
    const content = readFileSync(
      join(FIXTURES_DIR, "minimal-prompt.md"),
      "utf-8"
    );
    ...
    expect(result.prompt?.description).toBe(
  ...
  it("should fail for prompt with missing name", () => {
    const content = readFileSync(
      join(FIXTURES_DIR, "invalid-missing-name.md"),
      "utf-8"
    );
